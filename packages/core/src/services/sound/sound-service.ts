...
export class SoundService {
    ...
    #saveSettings(): void {
        ...
        try {
            ...
            const storageError = new StorageError('Failed to save sound settings to localStorage', {
                component: 'SoundService',
                method: '#saveSettings',
                originalError: error,
            });
            handleErrorSilently(storageError);
        ...
        try {
            const sound = new Howl({
                ...
                onloaderror: (id, error) => {
                    ...
                    const audioError = new AudioError(`Failed to load ${audioType} "${key}"`, key, {
                        component: 'SoundService',
            ...
            const audioType = type === 'music' ? 'music' : 'sound';
            const audioError = new AudioError(`Failed to register ${audioType} "${key}"`, key, {
                component: 'SoundService',
                method: '#registerAudio',
                originalError: error,
            });
    ...
    play(key: SoundKey, options?: { loop?: boolean; volume?: number }): number | undefined {
        ...
        const isEnabled =
            soundType === 'music' ? this.#soundSettings.music : this.#soundSettings.sound;
        ...
        try {
            ...
            const audioError = new AudioError(`Failed to play sound "${key}"`, key, {
                component: 'SoundService',
                method: 'play',
                originalError: error,
            });
